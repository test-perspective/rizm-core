...
pub(super) fn execute_admin_tool(
    ...
    match call.name.as_str() {
        ...
        "bulk_delete_users" => {
            tool_exec_admin_users::bulk_delete_users(state, user, &call.arguments)
        }
        ...
        "add_member_to_group" => {
            tool_exec_admin_groups::add_member_to_group(state, &call.arguments)
        }
        "remove_member_from_group" => {
            tool_exec_admin_groups::remove_member_from_group(state, &call.arguments)
        }
        ...
        "get_project_policy" => {
            tool_exec_admin_policy::get_project_policy(state, user, &call.arguments)
        }
        "grant_project_user_access" => {
            tool_exec_admin_policy::grant_project_user_access(state, user, &call.arguments)
        }
