...
pub(super) fn build_chat_system_prompt(
    ...
    s.push_str("Moving tasks between projects: use move_tasks. Subtasks move with their parent, task keys are re-issued in the destination project, and cross-project references are removed. Report the new keys and anything that was detached.\n");
