...
pub fn router() -> Router<AppState> {
    Router::new()
        ...
        .route(
            "/api/permissions/groups/:id/members",
            get(get_group_members).post(add_member),
        )
        .route(
            "/api/permissions/groups/:id/members/:user_id",
...
async fn list_groups(
    ...
    let groups = (state.db.read().await)
        .list_user_groups()
...
async fn create_group(
    ...
    let id = (state.db.read().await)
        .create_user_group(&req.name, req.description.as_deref())
        .map_err(|_| ApiError::internal())?;
    let group = (state.db.read().await)
        .get_user_group(&id)
        .map_err(|_| ApiError::internal())?
...
async fn update_group(
    ...
    (state.db.read().await)
        .update_user_group(&id, &req.name, req.description.as_deref())
        .map_err(|_| ApiError::internal())?;
    let group = (state.db.read().await)
        .get_user_group(&id)
        .map_err(|_| ApiError::internal())?
...
async fn get_group_members(
    ...
    let user_ids = (state.db.read().await)
...
async fn add_member(
    ...
    (state.db.read().await)
        .add_user_to_group(&req.user_id, &id)
        .map_err(|_| ApiError::internal())?;
    Ok(StatusCode::NO_CONTENT)
}
...
async fn remove_member(
    ...
    (state.db.read().await)
        .remove_user_from_group(&user_id, &id)
        .map_err(|_| ApiError::internal())?;
    Ok(StatusCode::NO_CONTENT)
}
...
async fn get_project_policy(
    ...
    if !can_read(&*state.db.read().await, &project_id, Some(&user))
        .map_err(|_| ApiError::internal())?
    {
        return Err(ApiError::forbidden("insufficient permissions"));
    }

    let policy = (state.db.read().await)
...
async fn update_project_policy(
    ...
    (state.db.read().await)
        .set_project_policy(&project_id, req.policy)
        .map_err(|_| ApiError::internal())?;
    Ok(StatusCode::NO_CONTENT)
}
...
async fn check_permission(
    ...
    let can_read_result = can_read(&*state.db.read().await, &params.project_id, Some(&user))
        .map_err(|_| ApiError::internal())?;
    let can_write_result = can_write(&*state.db.read().await, &params.project_id, Some(&user))
        .map_err(|_| ApiError::internal())?;
...
fn validate_policy(policy: &ProjectPolicy) -> Result<(), ApiError> {
    ...
    if policy.project_defaults.anonymous == crate::models::Permission::Write {
        return Err(ApiError::bad_request(
            "anonymous users cannot have write permission on project defaults",
        ));
    }
