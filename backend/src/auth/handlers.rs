...
pub fn router() -> Router<AppState> {
    Router::new()
        ...
        .route("/api/auth/dev-admin-login", get(dev_admin_login_status).post(dev_admin_login))
}
...
async fn login(
    ...
    let email = normalize_email(&req.email).ok_or_else(|| ApiError::unauthorized("invalid credentials"))?;
    ...
    let user = {
        ...
        db.get_user_by_email(&email).map_err(|_| ApiError::internal())?
    };
    ...
    let user = match user {
        None => {
            ...
            let _ = {
                let db = state.db.read().await;
                db.insert_audit_log(None, "LOGIN_FAILURE", None, Some(&json_meta(&email, &ip)), now)
            };
    ...
    let password_hash = user.password_hash.clone().ok_or_else(|| ApiError::unauthorized("invalid credentials"))?;
    ...
    .into_response();
    res.headers_mut()
        .append(header::SET_COOKIE, HeaderValue::from_str(&cookie.to_string()).map_err(|_| ApiError::internal())?);
    Ok(res)
}

async fn logout(State(state): State<AppState>, Extension(user): Extension<AuthedUser>) -> Result<impl IntoResponse, ApiError> {
    let now = crate::time::now_ms();
    {
        let db = state.db.read().await;
        let _ = db.delete_session(&user.session_id);
        let _ = db.insert_audit_log(Some(&user.user_id), "LOGOUT", Some(&user.user_id), None, now);
    }
...
async fn change_password(
    ...
    let hash = u.password_hash.ok_or_else(|| ApiError::bad_request("password not set"))?;
    ...
    let new_hash = super::utils::hash_password_for_bootstrap(&req.new_password);
    {
        ...
        let now = crate::time::now_ms();
        let _ = db.insert_audit_log(Some(&user.user_id), "PASSWORD_CHANGED", Some(&user.user_id), None, now);
    }
...
async fn dev_admin_login_status(State(state): State<AppState>) -> Result<impl IntoResponse, ApiError> {
...
async fn dev_admin_login(
    ...
    let user = {
        let db = state.db.read().await;
        db.get_user_by_email(&admin_email).map_err(|_| ApiError::internal())?
    };
    ...
    let user = match user {
        None => {
            let _ = {
                let db = state.db.read().await;
                db.insert_audit_log(None, "DEV_LOGIN_FAILURE", None, Some(&json_meta(&admin_email, &ip)), now)
            };
