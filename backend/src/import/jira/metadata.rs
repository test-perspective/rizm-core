...
pub async fn fetch_metadata(
    ...
    let statuses: Vec<ImportStatusMeta> =
        if let Some(proj) = project_id_or_key.filter(|s| !s.trim().is_empty()) {
            ...
            for it in project_statuses_json {
                ...
                for s in statuses_arr {
                    let Some(id_val) = s.get("id").and_then(|v| v.as_str()) else {
                        continue;
                    };
                    ...
                    let Some(name_val) = s.get("name").and_then(|v| v.as_str()) else {
                        continue;
                    };
    ...
    let issue_types: Option<Vec<String>> = if let Some(proj) =
        project_id_or_key.filter(|s| !s.trim().is_empty())
    {
